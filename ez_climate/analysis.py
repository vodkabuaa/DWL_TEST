...
def delta_consumption(m, utility, cons_tree, cost_tree, delta_m):
	...
	gs = GradientSearch(learning_rate=0.001, var_nums=len(m), utility=utility, fixed_values=fixed_values,
					    fixed_indicies=fixed_indicies, iterations=100, print_progress=True)
	...
	return new_cons_tree, cost_array, new_utility
...
def save_sensitivity_analysis(m, utility, utility_tree, cons_tree, cost_tree, ce_tree, prefix=None, return_delta_utility=False):
	...
	append_to_existing([[end_price], [perp_yield], [scaled_discounted_ed.sum()], [scaled_risk_premiums.sum()], [utility.cost.price(0, m[0], 0)]],
                       prefix+"sensitivity_output", header=["Zero Bound Price", "Perp Yield", "Expected Damages", "Risk Premium", "SCC"],
					   start_char='\n')
...
def save_constraint_analysis(cfp_m, utility, delta_util_x, delta_cons=0.01, prefix=None):
	...
	if prefix is not None:
		...
		prefix = ""
	write_columns_csv([delta_util_x, delta_util_c, [delta_con], marginal_benefit], prefix+"constraint_output",
					   header=["Delta Utility Mitigation", "Delta Utility Consumption", "Delta Consumption", "Marginal Benefit"])
