...
from ez_climate.tools import _pickle_method, _unpickle_method
try:
    import copy_reg
except:
...
class GeneticAlgorithm(object):
	...
	def run(self):
		...
		for g in range(0, self.num_gen):
			...
			self._uniform_mutation(pop_select, 0.50, np.exp(-float(g)/self.num_gen)**2)
